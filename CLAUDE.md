...
- **IaC CloudWatch-tag charset gotcha (sibling of the IAM one above):** `aws_cloudwatch_metric_alarm` (and CloudWatch tags generally) reject an em-dash in a **tag value** — `ValidationError: Tags can only contain letters, numbers, spaces, and the following special characters: _ . : / = + - @`. This charset is **stricter than IAM's**: no Latin-1 supplement, and NOT even `§` U+00A7. Same blind spot — `terraform validate` + `plan` PASS, only `apply` fails, and it PARTIAL-applies (in picasso#710 the `aws_cloudwatch_log_metric_filter` created but the alarm's `PutMetricAlarm` was rejected on the tag; fixed forward in #711). The CloudWatch **`alarm_description`** field is NOT charset-limited (it accepts em-dashes) — only **tags** are. So the pre-commit grep above should also cover `tags { ... }` **values** on non-IAM resources, not just `description`/`name`. (Cost a partial apply, 2026-07-05.)
