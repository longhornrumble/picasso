...
- **Owns:** delete `theme.css`, `useCSSVariables.js`, CSSVariablesProvider path, avatar pipeline, feature-display var machinery, Montserrat/Poppins leftovers; retire legacy `--*` names. **Status:** DONE (38fd567)
...
- **PR #672 note:** avatar pipeline, feature-display var machinery, and Montserrat/Poppins leftovers were already fully orphaned (zero live references found) — deleted along with `theme.css`/`useCSSVariables.js` with no separate work needed. **CRITICAL finding**: the excluded scheduling page (`?mode=schedule`, D8) derived its own `--sp-*` tokens from six old vars via `schedule-page.css` — preserved via a new minimal `useLegacySchedulePageBrandingVars.js` hook (byte-identical fallback semantics) rather than deleting it out from under that page. Bundle size: staging/production `iframe-main.js`+`iframe-main.css` combined -19.0%/-19.3%. Full jest green (49 suites/940 tests, +4 new). Live-verified via Playwright on the dev harness: welcome, thread+response-actions, composer expanded, settings, privacy, and the scheduling page all render correctly; Common Questions overlay/forms/in-chat scheduling/showcase not independently re-driven this pass (sandbox has no egress to the config Lambda — same limitation as PRs #650/#662) but covered by their own unaffected green Jest suites — W6.3 is the full per-tenant staging matrix.
