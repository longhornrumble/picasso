...
const getEnvironment = () => {
  ...
  // PRIORITY 2: Check build-time environment constant
  // When esbuild replaces __ENVIRONMENT__ with "staging", this check still works
  if (typeof __ENVIRONMENT__ !== 'undefined') {
    console.log(`🚀 Using build-time environment: ${__ENVIRONMENT__}`);
    return __ENVIRONMENT__;
  }

  // PRIORITY 3: Check if we're in a staging build (when running from localhost)
  // This handles the case where staging build is served from localhost
  if (typeof __IS_STAGING__ !== 'undefined' && __IS_STAGING__ === true) {
    console.log('🚀 Staging build detected via __IS_STAGING__ flag');
    return 'staging';
  }
  ...
  if (typeof window !== 'undefined') {
    try {
      ...
      // Auto-detect based on hostname for convenience (but not if we're a staging build)
      const hostname = window.location.hostname;
      // Don't override staging builds running on localhost
      if ((hostname === 'localhost' || hostname === '127.0.0.1' || hostname.startsWith('192.168.'))
          && typeof __IS_STAGING__ === 'undefined') {
...
const ENVIRONMENTS = {
  ...
  staging: {
    ...
    STREAMING_ENDPOINT: typeof __STREAMING_ENDPOINT__ !== 'undefined' ? __STREAMING_ENDPOINT__ : 'https://xqc4wnxwia2nytjkbw6xasjd6q0jckgb.lambda-url.us-east-1.on.aws', // Production Bedrock_Streaming_Handler (for staging E2E testing)
    STREAMING_METHOD: 'POST', // Lambda expects POST requests with JSON body
