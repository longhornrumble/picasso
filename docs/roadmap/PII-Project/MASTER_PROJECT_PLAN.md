...
| F-DSAR21 | Prod hand-managed IaC drift on PII surfaces (scope-expanded 2026-05-24 via M9.G4 audit-of-audit: + Sprint B placeholder-body anomaly + Python+Node writer-pair enumeration + audit-table CloudTrail data-events config) | OPEN M/L (Step 1 closed 2026-05-25 — uniform PITR drift on 5 surfaces classified (c) accept-with-named-trigger; Step 1.5 sub-items deferred) | **M9.G5 Step 1 CLOSED** via `tools/pii-prod-drift-audit.sh` + `prod-iac-drift-audit-2026-05-25.md`; Step 1.5+ (anomaly investigation, writer-pair enumeration, audit-table CT data-events, prod CT→CW Logs export) remain deferred-with-trigger under M9.G5 |
