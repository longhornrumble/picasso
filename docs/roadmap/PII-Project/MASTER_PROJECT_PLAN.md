...
**Status:** ✅ **DONE 2026-05-23** — both halves LIVE in staging + prod. Status bucket: ACTIVE → DONE.
...
| M9.G3 | **F-DSAR18 active-writer `pii_subject_id` gap** — BSH `form_handler.js` does NOT write `pii_subject_id`, so the DSAR walker `_walk_form_submissions` silently false-negatives on ALL post-Phase-1 widget chat-form rows (the active writer path). End-to-end DSAR completeness IS preserved via `manual_followup` fallback → operator email-Scan (existing F-DSAR1 procedure). Walker AUTOMATION value is degraded. Preferred fix: option (1) in F-DSAR18 = add `pii_subject_id` write to BSH `form_handler.js` Item dict (mirror Python writer pattern from PR lambda#142). Promotion-to-build triggers (any one): (a) operator Scan fallback used >3× total, (b) DSAR volume ≥1/month sustained, (c) prod-twin scoping starts, (d) Atlanta LOI fires. | F-DSAR18 (D5); discovered during M4 #2 functional verification 2026-05-23 | ~1d engineering when triggered |

**Closes:**
- D5: G-K (executes); F-DSAR1 / F-DSAR3 / F-DSAR4 (design-only stubs with named triggers); F-DSAR18 (when promoted via M9.G3 trigger).
