...
| M1.G6 | **F-DSAR18 active-writer `pii_subject_id` gap (RE-ROUTED here from M9.G3 per advisor review 2026-05-23 — §3 routing-rule violation: the gap touches M1's walker surface).** BSH `form_handler.js` does NOT write `pii_subject_id`; DSAR walker `_walk_form_submissions` filters on it → all post-Phase-1 active-writer rows silently false-negative. Manual_followup fallback (existing F-DSAR1 path) preserves end-to-end mechanism; walker AUTOMATION value for form-submissions surface is degraded. M1 status remains DONE but with explicit caveat: the form-submissions walker is procedurally degraded. Preferred fix: option 1 in F-DSAR18 (mirror Python writer pattern on BSH ~1d). **Tightened promote-to-build triggers (any one):** (a) **first form-submission DSAR ever fires** (replaces ">3× operator scans"), (b) DSAR volume ≥1/month sustained, (c) prod-twin scoping starts, (d) Atlanta LOI fires, (e) **calendar backstop 2026-06-22 (30 days from discovery — tightened per phase-completion-audit 2026-05-23)**. | F-DSAR18 (D5); advisor review 2026-05-23 (compliance 🔴-2+3 + lifecycle 🔴-2) | ~1d engineering when triggered |

**Closes (D5 + strategy-guide):**
- D5: G-F (partial; portability path delivered here, playbook completion in M3); F-DSAR1 (procedurally accepted; backfill trigger named via M1.G4); F-DSAR2 (residual L/H accepted; revisit triggers inherited via done-bar #7); F-DSAR3 (walker-mitigated; durable fix → M9); F-DSAR4 (TTL-mitigated; durable fix → M9); F-DSAR-C2-SSE-DEFER (binary decision per done-bar #6); F-DSAR18 (procedurally accepted via fallback; writer-side fix tracked via M1.G6 with named triggers).
...
**Status:** ✅ **DONE 2026-05-23** with **named-residual-deferrals captured per advisor review 2026-05-23**: done-bar #2 re-classified to **PARTIAL** (fault-test substituted FilterPolicy for original "disable SNS topic" wording without §3 user re-scope authorization; secondary control not empirically proven to fire independently of operator initiation) — tracked as new D5 row **F-DSAR22**. Other 5 done-bar items remain LIVE; M3's overall status stays DONE with the explicit caveat. Status bucket: ACTIVE → DONE (caveated).
...
- #2 (Fault-test): **LIVE ✅ 2026-05-23T16:58Z** — full fault-test executed using non-destructive SNS FilterPolicy approach (preferred over unsubscribe, which would require email re-confirmation). Set `FilterPolicy={"fault_test_block":["never_present"]}` on Chris's subscription; inserted at-risk row `smoke-sla-faulttest-001`; invoked Lambda → `at_risk_count=1`. SNS metrics for 16:58 bucket: `NumberOfMessagesPublished=1`, **no `NumberOfNotificationsDelivered` datapoint** — confirming publish succeeded but delivery was filtered (simulating the alarm-miss condition). Secondary check (playbook §8 weekly Monday CLI scan) found the row independently. FilterPolicy removed; synthetic row closed; subscription returned to normal-flow. G-D closure conditions met end-to-end. Earlier email-only test (8:14Z) verified normal-flow delivery; URL-fix test (16:51Z) verified clickable playbook URL post-PR #144 redeploy. Re-run cadence: annual (paired with quarterly D2/D3/D4 currency review per master plan §4) OR after changes to monitor Lambda / topic / subscription / IAM scope.
...
**Status:** ✅ **DONE 2026-05-23** with **named-residual-deferrals captured per advisor review + phase-completion-audit 2026-05-23**: (a) **F-DSAR18** active-writer `pii_subject_id` gap (writer fix tracked via M1.G6 — re-routed from M9.G3; backstop tightened to 2026-06-22 per phase-audit tech-lead 🔴); (b) **F-DSAR19** historical prod TTL backfill (tracked as **M4.G2** below; backstop tightened to 2026-06-22 per phase-audit tech-lead 🔴 — "every prod row since prod inception persists indefinitely; corrected widget claim now live contradicts that"); (c) **F-DSAR23** surviving widget bullet + per-`form_type` TTL matrix (tracked as **M4.G3** below; backstop tightened to 2026-06-22). **Done-bar #2 wording note per phase-audit code-reviewer 🟡:** the original wording references `Master_Function_Staging/form_handler.py:_store_submission` (dormant for the widget chat-form path); the **active writer is `Bedrock_Streaming_Handler_Staging/form_handler.js`** (lambda PR #145). Both writers were fixed; verification artifact is the BSH PR. Both M4 halves remain LIVE; M4's overall status stays DONE with the explicit caveats. Status bucket: ACTIVE → DONE (caveated).
...
| M4.G2 | **Historical prod `picasso_form_submissions` TTL backfill** (added per advisor review — lifecycle 🔴 #5). Prod TTL config was DISABLED pre-2026-05-23T18:01Z; enable is forward-only. All prod rows since prod inception persist indefinitely. One-shot backfill spec: read-only audit first (count + age-distribution), then per-age TTL assignment (rows >365d → now+30d eviction; 180-365d → now+90d; <180d → now+(365-age)d), then operator-approved one-shot execution. F-DSAR19 (D5) carries the full row. | F-DSAR19 (D5); advisor review 2026-05-23 (lifecycle 🔴 #5) | ~0.5d audit + ~0.5d backfill spec + ~0.5d execution + verification = ~1.5d total when triggered |
...
**Closes:**
- D5: G-A (both halves: widget claim + form-submission TTL) — closed with caveats per F-DSAR23 (surviving bullet) + F-DSAR19 (historical backfill) deferred via M4.G2 + M4.G3.
...
| M9.G6 | **M3 SLA monitor secondary-control independence** (added per advisor review — compliance 🔴-1+5 — F-DSAR22). **Per phase-completion-audit 2026-05-23 (tech-lead 🟡 + security 🟡 implicit): option 2 (belt-and-suspenders cron) PROMOTED from deferred-trigger to active-build** — a secondary control that requires operator initiation is not independent verification. Three options remain: (1) enforce Google Calendar recurring event (operator-side OAuth); **(2) add belt-and-suspenders cron — second EventBridge schedule emailing operator weekly to remember the CLI scan (~0.5d) — PREFERRED**; (3) re-do fault test with operator-blinded approach. M3 done-bar #2 stays PARTIAL until M9.G6 ships. Quarterly re-run cadence (replaces v0.7's annual). | F-DSAR22 (D5); advisor + phase-audit reviews 2026-05-23 | ~0.5d option 2 belt-and-suspenders cron — **PROMOTED to active backlog 2026-05-23** |
...
**Closes:**
- D5: G-K (executes); F-DSAR1 / F-DSAR3 / F-DSAR4 (design-only stubs with named triggers); F-DSAR20 / F-DSAR21 / F-DSAR22 (advisor review 2026-05-23 findings; closed when respective gap-router triggers fire); F-DSAR24 / F-DSAR25 / F-DSAR26 / F-DSAR27 / F-DSAR28 (phase-completion-audit 2026-05-23 findings; routed to M9.G7 + M9.G8).
