...
**Status:** ✅ **DONE 2026-05-23** with **named-residual-deferrals captured per advisor review 2026-05-23**: (a) **F-DSAR18** active-writer `pii_subject_id` gap (writer fix tracked via M1.G6 — re-routed from M9.G3); (b) **F-DSAR19** historical prod TTL backfill (new D5 row; tracked as **M4.G2** below; one-shot backfill spec scoped, calendar backstop 2026-08-23); (c) **F-DSAR23** surviving widget bullet "30 minutes session storage" misleading-by-omission post-365d-TTL + 365d TTL not differentiated by `form_type` (tracked as **M4.G3** below, batches with M8.G1 privacy notice). Both M4 halves remain LIVE; M4's overall status stays DONE with the explicit caveats. Status bucket: ACTIVE → DONE (caveated).
...
| M9.G6 | **M3 SLA monitor secondary-control independence** (added per advisor review — compliance 🔴-1+5 — F-DSAR22). Three options: enforce Google Calendar recurring event (operator-side OAuth); add belt-and-suspenders cron (~0.5d EventBridge schedule); re-do fault test with operator-blinded approach. Re-classify M3 done-bar #2 status to PARTIAL with this row as the open gap. Quarterly re-run cadence (replace v0.7's annual). | F-DSAR22 (D5); advisor review 2026-05-23 | ~0.5d (option 2: belt-and-suspenders cron) — preferred |

**Closes:**
- D5: G-K (executes); F-DSAR1 / F-DSAR3 / F-DSAR4 (design-only stubs with named triggers); F-DSAR20 / F-DSAR21 / F-DSAR22 (advisor review 2026-05-23 findings; closed when respective gap-router triggers fire).
