...
| M9.G5 | **Prod hand-managed IaC drift audit** (added per advisor review — compliance 🔴-4 — F-DSAR21). **Step 1 CLOSED 2026-05-25**: read-only audit script `tools/pii-prod-drift-audit.sh` shipped + executed against prod-614 (5 PII surfaces: form_submissions/notification-sends/notification-events/recent-messages/session-summaries; dimensions: TTL/PITR/SSE/DeletionProtection). **Findings**: uniform drift — PITR disabled on all 5 surfaces (staging IaC expects enabled); TTL/SSE/DeletionProtection all match expected state. Drift classified **(c) accept-with-named-trigger** per `prod-iac-drift-audit-2026-05-25.md` (rationale: PITR is DR/availability not privacy/compliance; mutation cost non-trivial; no active blast radius; recurring AWS cost; will self-resolve via prod-Terraform-cutover). Triggers (any one): first prod DSAR ever; first DDB data-loss incident on any of the 5 surfaces; prod-Terraform cutover; calendar backstop 2026-08-22 D2/D3/D4 currency review. **Step 1.5 (NOT addressed in this PR — sub-step targets recorded in decision doc):** Sprint B placeholder-body anomaly investigation; Python+Node writer-pair enumeration; audit-table CloudTrail data-events config; prod CT→CW Logs export for UpdateTimeToLive alarming. **Step 2 (ongoing)**: quarterly re-run of `pii-prod-drift-audit.sh` tied to D2/D3/D4 currency review. **Step 3 (deferred)**: prioritize PII tables when prod cuts over to per-resource Terraform. | F-DSAR21 (D5; Step 1 closed, Step 1.5 deferred); advisor review 2026-05-23; audit-of-audit 2026-05-24 | — (actual ~1h end-to-end for Step 1 script + run + decision doc + closure docs; vs ~0.5d original estimate — under-runs because audit logic was straightforward 5-surface enumerate + 4-dimension diff against IaC; 8th consecutive milestone showing 4-10x estimate-vs-actual under-run, recorded for 2026-08-22 hard-floor review per `feedback_milestone_estimate_calibration`) |
