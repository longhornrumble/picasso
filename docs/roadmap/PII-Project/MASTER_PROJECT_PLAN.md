...
**Status buckets:**
- **DONE**: M1 (2026-05-23), M3 (2026-05-23)
- **ACTIVE** (build now): M2, M4, M9
...
**Status:** ✅ **DONE 2026-05-23** — all 6 done-bar items closed. Status bucket: ACTIVE → DONE.
...
**Status:** **STAGING FULLY VERIFIED 2026-05-23** — both halves LIVE + functionally tested end-to-end in staging acct 525. Prod (614) promotion = separate operator decision.

**Closure (per-item):**
- #1 (Widget claim removed): picasso PR [#172](https://github.com/longhornrumble/picasso/pull/172) MERGED to staging branch. **STAGING LIVE 2026-05-23T17:08Z**: `npm run build:staging` + `aws s3 sync dist/staging/ s3://picasso-widget-staging/` + CF `E3G30AUOEJTB36` invalidation `I9LYUMWR0O94OZ1UVAT26ASYTZ` (Completed). Verifiable: `curl staging.chat.myrecruiter.ai/iframe-main.js | grep -c 'No personal information stored permanently'` returns 0; Last-Modified now `Sat, 23 May 2026 17:08:21 GMT` (was 4 days stale pre-deploy). **PROD `chat.myrecruiter.ai`** operator-pending: requires staging→main promotion in picasso repo + `deploy-production.sh` execution (uses `chris-admin` profile + `s3://picassocode/` + CF `E3G0LSWB1AQ9LP`). Stall-trigger NOT hit (PR opened + merged + staging-deployed same day as M1 closure 2026-05-23).
- #2 (form_handler TTL writer): **STAGING FUNCTIONALLY VERIFIED 2026-05-23T17:37Z.** Initial assumption that `Master_Function_Staging/form_handler.py` was the active writer turned out to be wrong — empirical verification via real staging form submission showed `submission_id=form_contact_1779556915439` (BSH `<form_id>_<ms-timestamp>` naming) and **no ttl attribute**. The actual active writer for widget chat-form submissions is `Bedrock_Streaming_Handler_Staging/form_handler.js` (Node.js). PR [#142](https://github.com/longhornrumble/lambda/pull/142) fixed the dormant Python writer (still correct + tested); **PR [#145](https://github.com/longhornrumble/lambda/pull/145) fixed the actual active Node.js writer** (mirrors the 365d ttl on BSH put_item Item; jest test asserts presence + window). CI auto-deployed BSH CodeSha256 `KPBNLMSrlaEf72R54a1jBbNFnj4jALhot0/aQMJ0Ahs=` 17:28:39Z; subsequent test submission `form_contact_1779557864165` written with `ttl=1811093864` = exactly `now+31,536,000s` (365 days) ✅. Pre-fix submission (`form_contact_1779556915439`) remains in-place without ttl — append-only schema means no backfill; acceptable for one row (will not be revisited).
- #3 (Both halves re-verified in prod): **OPERATOR-PENDING** — requires (a) Picasso staging→main promotion + `deploy-production.sh`, (b) BSH (lambda) staging→main promotion + hand-managed Bedrock_Streaming_Handler deploy to prod-614, then (c) re-run Phase 0.5 Step 5 verification + a synthetic prod form submission to confirm `ttl` is written.
...
**Closes:**
- D5: G-A (both halves: widget claim + form-submission TTL).
...
**Closes:**
- D5: G-K (executes); F-DSAR1 / F-DSAR3 / F-DSAR4 (design-only stubs with named triggers).
