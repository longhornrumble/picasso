...
**Status:** **IN PROGRESS** (5 of 6 done-bar items LIVE or substantially LIVE; #3 Gmail config + #5 prod ARCHIVE remain operator-pending).
...
- #2 (Fault-test): **PARTIAL ✅** — agent-executed safe test-fire 2026-05-23T08:14Z: synthetic `request_received` row 30d ago inserted → manual Lambda invoke returned `at_risk_count=1` + CloudWatch log confirms SNS publish attempted (run `bf14fe73-4aae-...`); subsequent closed-event PutItem + re-invoke returned `at_risk_count=0` confirming skip-on-closed (run `bead2ada-...`). Email-receipt confirmation by operator (email subscription to `picasso-ops-alerts-staging` topic) closes the loop. Full fault-test (disable SNS subscription to test secondary check) deferred to operator (disabling SNS is operationally disruptive to other ops alarms; weekly Monday CLI per playbook §8 already provides secondary check).
- #3 (Gmail config): **OPERATOR-PENDING** — agent attempted Gmail MCP `create_label` for `dsar/open`, `dsar/awaiting-verification`, `dsar/closed` 2026-05-23; failed with "insufficient authentication scopes" (Gmail MCP scope grants `list_labels` but not `create_label`). Email alias `privacy@myrecruiter.ai` creation requires Google Workspace admin Console (no API for individual users). Filter creation requires Gmail Web UI (no MCP tool exists). All three sub-items are operator-side via Google Workspace admin + Gmail Web UI.
...
- #5 (ARCHIVE CLI verification): **STAGING LIVE ✅; PROD OPERATOR-PENDING** — agent retry of prod `lambda list-functions` blocked by auto-mode classifier even after user-explicit re-authorization 2026-05-23 (race with AskUserQuestion processing pipeline; classifier inconsistency — bedrock prod read succeeded, lambda prod read blocked within same authorization window). Operator runs the prod ARCHIVE_BUCKET lookup + posture checks per the decision doc CLI; expected outcome: prod analogue with matching posture. Staging finding (F-DSAR17) remains routed to M9 TTL hygiene audit.
...
**Status:** **IN PROGRESS — staging half LIVE for one of two halves; operator-pending for both prod halves + Picasso staging redeploy.**

**Closure (per-item):**
- #1 (Widget claim removed): picasso PR [#172](https://github.com/longhornrumble/picasso/pull/172) MERGED to staging branch. **Picasso bundle NOT YET REDEPLOYED**: `staging.chat.myrecruiter.ai/iframe-main.js` + `chat.myrecruiter.ai/iframe-main.js` both Last-Modified 2026-05-19 (4 days stale); `grep -c 'No personal information stored permanently'` still returns 1 in both. Operator-pending: Picasso build (`npm run build:staging`) + deploy (per CLAUDE.md SOP) → staging widget will then reflect the removal. Stall-trigger NOT hit (PR opened + merged same day as M1 closure 2026-05-23).
- #2 (form_handler TTL writer): lambda PR [#142](https://github.com/longhornrumble/lambda/pull/142) MERGED to main. **LIVE in staging acct 525**: `Master_Function_Staging` CodeSha256 `s3WIrLPMOy9NwVqKOPhup2J2GQrEzQJrzbW1LqUf6eU=` deployed 2026-05-23T08:02:25Z via CI auto-deploy. 109/109 DSAR tests + new MagicMock TTL writer test + DSAR walker contract test all PASS. **Prod (614) `Master_Function`** is hand-managed; promotion is a separate operator decision.
- #3 (Both halves re-verified in prod): **OPERATOR-PENDING** — requires (a) Picasso staging build + deploy + staging→main promotion to prod, (b) Master_Function_Staging → Master_Function prod promotion, then (c) re-run of Phase 0.5 Step 5 verification (`curl chat.myrecruiter.ai/iframe-main.js | grep` expected 0 + synthetic form submission to confirm `ttl` written in prod table).
