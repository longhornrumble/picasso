...
| **Pseudonymized session summary** | `picasso-session-summaries` (redacted `first_question` + counts/outcome; linkable by `pii_subject_id`) | **365 days in DDB** — operator decision 2026-06-02 (DDB-only 12-month queryable window); `TTL_DAYS`/`ARCHIVE_TTL_DAYS` 90→365 merged in lambda#207, **deploys on the next MFS/BSH/Dashboard release** | DDB-only: dashboard reads DDB for the full 365d in both envs (prod has no archive). TTL change applies to new rows; the 12-month window builds forward. Still retained redacted in the Glacier log archive |
...
| **Summary 12-month window** | ✅ **DECIDED + code-complete (DDB-only)** — `analytics_writer.TTL_DAYS` (py + js) **and** `Analytics_Dashboard_API.ARCHIVE_TTL_DAYS` 90→365 moved in lock-step (lambda#207, 121 tests pass). **Deploys on the next MFS/BSH/Dashboard release** (rides their cadence — those Lambdas carry other programs' undeployed main work, so no isolated deploy). Still pairs with the session-summaries DSAR walker (F-DSAR31) for the widened window | ✅ done (code) | M9 |
