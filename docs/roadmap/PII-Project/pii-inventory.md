...
**Scheduling Lambdas:** **landing in progress (sub-phase B).** `Calendar_Watch_Listener` (row above, lambda#161/#165/#173), `Calendar_Watch_Onboarder` (B5, picasso#261), and `Calendar_Watch_Renewer` (B3, lambda#172) now exist; their IaC lives in `infra/modules/lambda-calendar-watch-*-staging/`. This PR adds the `Calendar_Watch_Listener` §A row for the Phase 2b attendee-email→SQS surface. **Follow-up (named, not silently deferred):** a fuller scheduling-inventory refresh — §A rows for the Onboarder + Renewer, the `picasso-calendar-watch-events-staging.fifo` queue as an explicit sink, and any §D CloudWatch log groups — should land when those surfaces' PII shape is next touched (the Onboarder/Renewer handle calendar/OAuth metadata, not consumer transcript PII; the highest-PII surface is the Listener's attendee-email dispatch captured here).
