...
| **`picasso-channel-mappings-staging`** | PK `PK` · SK `SK`; GSI `TenantIndex(tenantId / channelType)` | Meta `pageId`, PSID-scoped records, `lastUserMessageAt`, **encrypted page tokens** (via `kms-channel-tokens-staging` CMK) | `ttl` / enabled / (writer-set) | on | off | SSE-DDB at table level; **page tokens encrypted by `kms-channel-tokens-staging` at item level** | Meta_Response_Processor, Meta_OAuth_Handler | Meta_* exec roles; future delete role | **DELETE** | Page tokens are sensitive credentials (Meta OAuth); item-level CMK pattern is the live model for what Apply-2 may extend to other surfaces. |
...
| `picasso-calendar-watch-channels-staging` | PK `channel_id`; GSIs `tenant-status-index(tenant_id, status)` + `tenant-expiration-index(tenant_id, expiration)` | `calendar_id` (coordinator workspace email — tenant operator identity); `channel_token` (per-channel 64-char hex security secret — **Tier-4** per `data-classification.md` Rule 4) | (no TTL — lifecycle managed by Renewer per B3 and onboarding/offboarding hooks per B5/B6) | **on** (enabled 2026-05-25 per audit closure row 2; PITR is the standard for any table holding a Tier-4 secret per max-tier rule) | off | SSE-DDB at table level. **`channel_token` item-level encryption deferred to B2 implementation gate** (decision: CMK item-level encryption per `picasso-channel-mappings-staging` precedent OR move `channel_token` to Secrets Manager and store SHA-256 hash in DDB). B2's Security-Reviewer pass cannot close without addressing this. | **No writer Lambda yet** (B2/B3/B5/B6 not implemented) | TBD when B2/B3/B5/B6 Lambdas land | **NOT-CONSUMER** ⚠ (`calendar_id` is workspace email of coordinator — for tenants where coordinators are paid staff, NOT-CONSUMER is settled; for tenants where coordinators are volunteers — likely under CPRA — the same G-H counsel-input gap as the `picasso-employee-registry-v2-staging` row above applies. Do not assert as settled for volunteer-coordinator tenants without counsel input.) | Calendar watch channel ledger (canonical §14.1). Per [`scheduling/docs/subphase_b1_calendar_watch_channels_runbook.md`](../../../scheduling/docs/subphase_b1_calendar_watch_channels_runbook.md). **Max-tier rule applies:** `channel_token` is Tier-4; entire row is Tier-4 by `data-classification.md` Rule 4. |
