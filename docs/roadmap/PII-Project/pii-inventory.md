...
| `picasso-calendar-watch-channels-staging` | PK `channel_id`; GSI `tenant-status-index(tenant_id, status)` | `calendar_id` (coordinator workspace email — tenant operator identity, not consumer); `channel_token` (security secret, not PII) | (no TTL — lifecycle managed by Renewer per B3 and onboarding/offboarding hooks per B5/B6) | off (created via B1 runbook 2026-05-25; PITR enable deferred until B5 starts writing real rows — pilot-scale low-priority) | off | SSE-DDB | **No writer Lambda yet** (B2/B3/B5/B6 not implemented) | TBD when B2/B3/B5/B6 Lambdas land | **NOT-CONSUMER** | Calendar watch channel ledger (canonical §14.1). Coordinator emails = tenant operator data, not consumer PII; same tier as `picasso-employee-registry-v2-staging`. Per [`scheduling/docs/subphase_b1_calendar_watch_channels_runbook.md`](../../../scheduling/docs/subphase_b1_calendar_watch_channels_runbook.md). |
