...
**Phase 0.5 deliverable D5.** Established 2026-05-20. Table form, **15 rows**, **pre-seeded** with the 11 gaps from the 2026-05-19 plan-gaps review (G-A..G-K) plus 4 new rows for the highest-priority D2+D3+D4-surfaced findings that don't map cleanly into a G-row. Each row carries a **named owner**; no unmitigated H/H row is ownerless.
...
| **F9** | **Tenant-controlled downstream destinations** (n8n / Sheets / CRMs / webhooks + per-tenant conditional S3 fulfillment buckets). Full Tier 3 form PII leaves the platform to destinations with no platform-controlled DPA, no encryption assurance, no delete reachability. **Donor-class submissions (D4 rule 8 confidentiality) are a distinct mitigation framing.** | D2 Finding 9 + D2 Finding 8 (per-tenant fulfillment); D3 §G; D4 rule 8. | 3 (both content + confidentiality origins) | **H / H** | None — tenant-config-driven; opaque from platform view. | Vendor inventory + delete-reachability assurance absent (second-wave deliverable, deferred). | **Chris** + **Counsel-pending-on-trigger** (S4 supplementary in PR #153 for KB-adjacent hygiene; donor-disclosure question S5 in PR #153) | **Decision A input at Step 10** — if PII leaves platform at submit, Path A delete pipeline is bounded by construction. Vendor inventory is a second-wave deliverable, named in Step 11 closeout (not built in Phase 0.5). |
...
- ✅ **≤ 15 rows total** (15 rows: 11 G-rows + 4 F-rows).
