...
| 2026-06-13 | **C7 staff-hours = measured interaction-event span (idle-capped), not message gaps** | Live E2E: the widget never emits MESSAGE_SENT/USER_MESSAGE, so the original "sum of message gaps" left staff-hours AND after-hours structurally 0. After-hours now keys off CONVERSATION_STARTED (= C7's "first user message"); active time sums consecutive gaps across the real interaction-event stream (CONVERSATION_STARTED + engaged/form/scheduling/chip events), 5-min cap + 1-min floor preserved. **OPERATOR RATIFICATION PENDING.** (lambda#325) |
| 2026-06-13 | **C7 engaged: `≥2 user messages` branch is inert** (no per-message events). Engaged rests on {CTA_CLICKED, LINK_CLICKED, FORM_VIEWED}. NOT silently expanded — **OPERATOR DECISION** whether to add ACTION_CHIP_CLICKED / SCHEDULING_* / a first-message proxy. | live-E2E finding |
| 2026-06-13 | **C4b repoint action exposed** (`PATCH /links/ext_{id}`, NEW error code NOT_FOUND→404); mint role gained `dynamodb:UpdateItem` | dynamic-QR destination change proven live (lambda#324, infra picasso#565) |
