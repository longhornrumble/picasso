...
> ✅ **EXECUTED 2026-06-05 (§P1–P6 + §P5.2 backfill).** Run with explicit operator authorization (the standing hard-stop was lifted for this runbook only). Created in prod 614: `picasso-pii-subject-index`, `picasso-pii-dsar-audit`, `picasso-pii-tenant-purge-audit` (both audit tables immutable), `picasso-sms-usage` (D4); roles `picasso-pii-dsar-role` + `picasso-pii-tenant-purge-role` (KMS grant omitted — prod data tables use default SSE); functions `picasso-pii-dsar` + `picasso-pii-tenant-purge` (reserved=1) with operator-only invoke. **Smoke PROVED the D2 prod single-key path LIVE** (DSAR access + purge dry-run both matched a seeded `form-submissions:1` via the `tenant-timestamp-index` GSI; audit DeleteItem denied even to admin). **§P5.2 backfill:** 47 rows stamped, 22 distinct subjects indexed, `post_condition_pass`, 0 unresolved (log: `docs/runbooks/ga3-subject-index-backfill-prod-20260605.md`). **STILL PENDING:** §P5.1 (prod BSH/MFS writer deploy — future-row auto-indexing, high blast radius) + §P7 (dashboard prod purge UI).
