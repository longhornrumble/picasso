...
// AMENDED 2026-06-12 (deterministic pipeline, lambda#298 / picasso FE companion): the widget's
// slot-chip / confirm clicks send `scheduling_action` (+ `scheduling_slot_id`) in routing_metadata;
// the flow consumes that signal DETERMINISTICALLY before the LLM detector (which remains the
// typed-text fallback). All §B14 guards unchanged (transition + persisted-candidate validation).
// select_slot success with identity resolved emits `scheduling_confirm`
//   { type:'scheduling_confirm', session_id, slot:{slotId,label}, attendee_email } —
// the SERVER-driven confirm affordance (FE renders SchedulingConfirmCard; no local confirm).
...
// bootstrap) and re-supplies `deps.qualifyingContext`.
// AMENDED 2026-06-12 (deterministic pipeline): identity is collected DETERMINISTICALLY — a bare
// email-shaped message for an in-flight `confirming` session is captured by the integrator-owned
// captureAttendeeEmail (persists `attendee_email` on the session row; saveState whitelist extended;
// shape-revalidated on read; form-injection attendee still WINS). Capture NEVER commits (§B14:
// free text never commits) — it re-arms the scheduling_confirm card; the user taps to commit. `invokeProposal` MAY run before identity is known
...
// AMENDED 2026-06-12 (integrator ratification, lambda#293 audit N-finding): the SSE message ALSO carries the
// codebase-conventional `session_id` top-level key (as scheduling_slots/scheduling_notice do). Consumers tolerate it.
```
...
//         (available_variables is PER-MOMENT — each moment exposes only the variables valid for it; it is NOT a top-level field.)
...
// AMENDED 2026-06-12 (lambda#294 security audit SR-1/SR-2):
//   1. PURPOSE CLAIM — the disconnect mint adds `purpose:'disconnect'` to the init-token payload and the
//      /connection/disconnect route REJECTS tokens lacking it (generic 400). A browser-visible connect-URL token is
//      structurally invalid against disconnect (cross-purpose replay closed). /connect + /connection/status unchanged.
//   2. WATCH SEMANTICS — the Offboarder invoke is async (Event type): success means DISPATCHED, so the response's
//      `watch` field is 'pending' (not 'stopped') on the happy path; SDK throw → 'none'. 'stopped' is reserved for a
//      future RequestResponse confirmation. FE treats 'pending' as success-with-cleanup-in-progress.
```
...
| 2026-06-13 | **§G LOCKED — gap-closure (G1–G5) additive fields (integrator).** All OPTIONAL + forward-compatible (absent → byte-identical to pre-gap). AppointmentType `agenda` (G2) → guest `.ics` DESCRIPTION + Google event description. employee-registry-v2 `availability_windows` (G4, per-resource intersect) + `max_bookings_per_day` (G5, integer, slot-gen day-cap). picasso-scheduled-messages `reschedule_url`/`cancel_url`/`join_url`/`when_label` (G1, minted in BCH, appended outside the editable body). Shipped lambda#327 (ENGINE) + lambda#328 (MESSAGING) → main, staging-deployed. Adversarial-audited (4 reviewers); see §G section + `project_scheduling_gap_build_phase_audit_2026-06-13.md`. |

---

## G. Gap-closure (G1–G5) additive fields — **LOCKED 2026-06-13** (shipped: lambda#327 ENGINE, lambda#328 MESSAGING; staging-deployed)

Every field below is **optional + forward-compatible** — old-shape records (and the no-field default path) behave byte-identically to pre-gap. Set via Sandbox/config for now (UI deferred to the operator's pass). No new PII *category* (config text + availability + capability URLs the reminder row already analogizes) → no `pii-inventory.md` change.

### G2 — `AppointmentType.agenda` (plain text)
Optional field on the AppointmentType row (§A). Rendered into BOTH (a) the guest `.ics` DESCRIPTION (`Booking_Commit_Handler/confirmation-email.js`, via `escapeIcsText` — escapes lone `\r` to block ICS-property injection) and (b) the Google API event description (`calendar-events.js`, via `clean()`). NOT `{{var}}`-templated in v1. Absent → today's description byte-identical. BCH self-sources it (best-effort `candidate-resolver.defaultGetAppointmentType`) when the commit event omits it.

### G4 — `employee-registry-v2.availability_windows` (per-staff hours)
Optional, same shape as the appt-type windows: `{ sun:[{start:"HH:MM",end:"HH:MM"}], mon:[…], … }` (day keys sun..sat; 24h "HH:MM"). Carried onto the candidate by `candidate-resolver.resolveCandidates` (projection + carry). In `pool.select`'s per-resource loop: **effective windows = staff ∩ appointment-type windows** (`intersectAvailabilityWindows` in `pool.js`; can only NARROW the admin envelope), then ∩ Google free/busy as today. Semantics: `null`/`undefined` staff windows = no constraint (identity — the exact `normalized` object is passed unchanged, enforced by a strict-`toBe` regression test); `{}` = explicitly unavailable (all days blocked). `slots.js` is UNCHANGED.

### G5 — `employee-registry-v2.max_bookings_per_day` (per-staff day cap)
Optional **positive integer** (fractional/NaN/string → omitted, treated as no cap). Carried onto the candidate (`candidate-resolver`). When set, `pool.select` excludes days already at/over cap for that resource from slot generation — booking count via `tenantId-coordinator_email-index` (`countBookingsByDay`, business-tz civil-day bucketed; FilterExpression scan, pilot-OK). Absent → no count query → byte-identical. **Residual race** (v1 guardrail): slot-gen exclusion is not a commit-time hard fence; a propose→commit race could admit the (cap+1)th. Commit-time fence = follow-up.

### G1 — `picasso-scheduled-messages` reminder-row action fields
Optional `reschedule_url`, `cancel_url`, `join_url`, `when_label` (strings) on the §E1 reminder write shape. **Minted on the COMMIT path** (`Booking_Commit_Handler` reuses `confirmation-email.buildActionLinks` — §B4 signed `cancel`/`reschedule` tokens) and on the reschedule path (`scheduling-mutate` → `rebindReminders`, fresh tokens for the new start_at), then passed into `Reminder_Scheduler/scheduler.js` as params. **`scheduler.js` imports NO signing SDK** (tokens.js's top-level `@aws-sdk/client-secrets-manager` is not a Reminder_Scheduler dependency — importing it breaks CI/runtime there). `whenLabel` is pure Intl (always computed). Fail-soft: a mint failure omits the links (reminder still ships with time + join). `Scheduled_Message_Sender` appends a rendered action block **OUTSIDE the editable/override body** (STOP-footer pattern) for ATTENDEE-facing reminders only — https-scheme-gated hrefs, STOP preserved exactly once, attendance (coordinator) rows excluded. The shared `notif-defaults.js` REMINDER_TEMPLATES are UNCHANGED (no ADA-parity break).
