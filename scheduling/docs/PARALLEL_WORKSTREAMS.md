...
| C8 (Wave 2) | feature/scheduling-ws-c8 | [#190](https://github.com/longhornrumble/lambda/pull/190) | MERGED 2026-05-31 | `Booking_Commit_Handler/` keystone — 4-reviewer audit + verified remediation (SDK timeouts via `aws-client-config`, lock TTL, lock-release-before-email, PII-log hashing, Zoom refresh-rotate/401, ICS DTSTAMP/SEQUENCE, branch gate→90). **Unblocks B9/B10/B11.** **Integrator-owed (next):** IaC module+IAM (timeout 90s/mem 512/reserved-conc 5/DLQ/per-tenant secret scope), pii-inventory (Booking writer, +C6/C9 rows; PII-session-coordinated), token-min-lifetime floor (tokens.js + §B4), `@googleapis/calendar` pin, operator-supervised live smoke (gates kanban→Done). |
