...
| WS-CAL-LIFECYCLE (Wave 4) | — | — | **PLANNED (I2-A, operator 2026-06-01)** | The §14.2 calendar-as-SoR reconciliation consumer the dispatch interface assumed exists but was never built: consumes `calendar_deleted`/`calendar_moved`/`calendar_reassigned`/`event_made_private` → reconcile the Booking row (`calendar_deleted`→`canceled`, etc.) + notify the volunteer. **B11's cancel path depends on it.** Needs contract (Y). Work-order to be drafted. |
| WS-SCHED-FOUNDATIONS (Wave 4) | — | — | **PLANNED (I6 + X/Y)** | **(X)** booking/coordinator→candidate-pool resolver (appointment_type→routing_policy→employee-registry→candidates — unblocks B9 reoffer + B11 reassign-roster + re-pooling) + **(Y)** volunteer-notification dispatch (reoffer / reschedule / cancel notice — front end of the sub-phase-E notification system). Likely sequenced with sub-phase E. |
