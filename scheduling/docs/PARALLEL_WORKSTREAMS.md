...
| WS-WIDGET (B-minimal) | [#359](https://github.com/longhornrumble/picasso/pull/359) | **IN REVIEW** | widget forwards opaque `?session=` → backend. LIGHT audit: reviewed clean (mirrors the `nocache` host→iframe→backend precedent 1:1; opaque, tenant from config; 291/291). **Two-hop surface confirmed** (iframe). Ready to merge. |
...
| WS-CONVO (B-minimal) | feature/scheduling-ws-convo | — | **READY (keystone — deps merged 2026-06-03)** | BSH `scheduling/schedulingFlow.js`+`bindingContext.js`+index.js wiring — reschedule/cancel flow; §B14 action-boundary (execute only on a focused-post-stream structured action, never free-text). **Now LAUNCHABLE** (FACADE/BINDING/ZOOM merged; consumes §B13/B12/B15 + §B9 execute + §B3 slots + C9 stateMachine). Note the §B10 session-id threading invariant. FULL audit (commit path + LLM boundary). |
