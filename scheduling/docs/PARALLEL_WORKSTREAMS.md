...
| WS-EUI | feature/scheduling-ws-eui | — | SCOPED — ready to build | **Integrator-scoped 2026-05-30 to E12 + E15 only** (render-only from §A + WS-FIX fixture), home = `picasso-analytics-dashboard/src/{pages,components}/scheduling/` (net-new only; base staging). **E11/E13/E14/E16 deferred** to integrator-sequenced follow-ups (backend/Security/edit-existing). |
