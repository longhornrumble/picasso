...
| WS-CAL-LIFECYCLE (Wave 4) | — | — | **READY** ([work-order](workstreams/WS-CAL-LIFECYCLE-calendar-reconciliation.md)) | §14.2 reconciliation consumer (2nd SQS consumer): `calendar_deleted`→`canceled`(+Y reschedule notice); `calendar_reassigned`→update `resource_id` (no notify, agent-of-CoR §5.1); `calendar_moved`→**v1: mark canceled + `rescheduleOfBookingId`, NO auto-rebook** (flagged); `event_made_private`→**channels-table** `status=event_body_private` (not a Booking write) + admin alert. (Y) stubbed until WS-SCHED-FOUNDATIONS. **B11's cancel path depends on it.** lambda·base main·HIGH-RISK. |
| WS-SCHED-FOUNDATIONS (Wave 4) | — | — | **READY** ([work-order](workstreams/WS-SCHED-FOUNDATIONS-pool-resolver-and-notify.md)) | **(X)** `shared/scheduling/candidate-resolver.js` — routing_policy.tag_conditions + employee-registry-v2 → `{resourceId,scheduling_tags,coordinatorEmail}` feeding the existing `routing.evaluatePool` (unblocks B9 reoffer + B11 roster seam). **(Y)** `shared/scheduling/notify.js` — `dispatchVolunteerNotice({kind})` wired to existing `send_email`/`notification_hub`, agent-of-CoR-guarded; SMS path stubbed if unreachable. lambda·base main·HIGH-RISK. |
