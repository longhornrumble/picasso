...
| WS-D7 (Wave D-core) | feature/scheduling-ws-d7-cancel | [lambda#203](https://github.com/longhornrumble/lambda/pull/203) | **IN REVIEW** | `shared/scheduling/cancel.js` — §B9 `executeCancel`, `events.delete` + `pending_calendar_sync`; **does NOT flip status** (cal-lifecycle listener owns that, already built). CI green (18 tests); integrator review clean (no-status-flip + no-ddb + PII-safe-log + immutability all asserted). **§C escalation RESOLVED:** §B9 calendar-facade shape pinned to `deleteEvent(booking)` (matches #203 — no rework). FULL audit (Security pass) running → **awaiting operator go-ahead to merge** (calendar-mutation surface; no auto-merge). In-chat caller = glue. |
