...
| C8 (Wave 2) | — | — | ⏳ READY pending Zoom OAuth | **C6 ✅ merged** — C8's only remaining gate is **Zoom S2S OAuth provisioning** (operator, `runbooks/ZOOM_OAUTH_PROVISIONING.md`). Meet-first + NullConferenceProvider buildable now; ZoomProvider once the secret exists. C8 is the keystone (writes Booking rows w/ `extendedProperties.private.booking_id`) → unblocks B9/B10/B11. |
