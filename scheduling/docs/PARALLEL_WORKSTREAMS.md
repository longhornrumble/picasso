...
| WS-FACADE (B-minimal) | feature/scheduling-ws-facade | — | READY | `shared/scheduling/calendarFacade.js` — §B13 `buildCalendarFacade` (curries per-tenant OAuth into calendar-events; = the §B9 `deps.calendar`). FULL audit (auth-bound). Parallel. |
| WS-BINDING (B-minimal) | feature/scheduling-ws-binding | — | READY | `shared/scheduling/sessionBinding.js` — §B12 `resolveBinding` (reads the §B10 row; TTL-in-code; tenant-from-context). FULL audit (gates booking ownership). Parallel. |
| WS-WIDGET (B-minimal) | feature/scheduling-ws-widget | — | READY | widget forwards `?session=` → backend (tenant from config). LIGHT. Parallel. **FIRST step: confirm the widget param/request surface (not pre-mapped) — STOP+flag if it differs.** |
| WS-ZOOM (B-minimal) | feature/scheduling-ws-zoom | — | READY | `Booking_Commit_Handler/zoom-client.js` +`updateMeeting` (§B15 start-time PATCH; mirror siblings). FULL audit (external). Parallel; no other slice touches the file. |
| WS-CONVO (B-minimal) | feature/scheduling-ws-convo | — | **BLOCKED (keystone)** | BSH `scheduling/schedulingFlow.js`+`bindingContext.js`+index.js wiring — reschedule/cancel flow; §B14 action-boundary (execute only on a focused-post-stream structured action, never free-text). **Launch ONLY after WS-FACADE+WS-BINDING+WS-ZOOM merge** (consumes §B13/B12/B15 + §B9 execute + §B3 slots + C9 stateMachine). FULL audit (commit path + LLM boundary). |
