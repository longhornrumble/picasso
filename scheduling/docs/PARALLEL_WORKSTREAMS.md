...
| C8 (Wave 2) | feature/scheduling-ws-c8 | [#190](https://github.com/longhornrumble/lambda/pull/190) (code) · picasso[#317](https://github.com/longhornrumble/picasso/pull/317) (IaC) · lambda[#191](https://github.com/longhornrumble/lambda/pull/191) (deploy) | MERGED + PROVISIONED 2026-05-31 | Code `Booking_Commit_Handler/` (4-reviewer audit + verified remediation). **IaC #317 applied on staging** (Terraform success: Lambda + dedicated least-privilege IAM role — Booking 4-verbs + RoutingPolicy UpdateItem + per-tenant OAuth/Zoom/JWT secrets + KMS DLQ; timeout 90s/mem 512/reserved-conc 5; **function runs the placeholder zip** per `ignore_changes`). **Deploy wired (#191).** **Unblocks B9/B10/B11.** **OPERATOR RESIDUAL (gates kanban→Done):** initial C8 code-deploy (`gh workflow run "Deploy Lambda — Staging" -f lambda=Booking_Commit_Handler`) + live smoke (Calendar/Zoom/SES/DDB; confirm `SCHEDULE_BASE_URL`). **Still-owed integrator follow-ups:** pii-inventory (Booking writer + C6/C9 rows, PII-coordinated), token-min-lifetime floor (tokens.js + §B4), `@googleapis/calendar` pin; Zoom prod published-OAuth pivot (pre-tenant-2). |
| B9 / B10 / B11 | — | — | ✅ UNBLOCKED (C8 merged) | Consumer logic deferred from sub-phase B (OOO / accept-decline / stranded-booking); C8 now writes real Booking rows w/ `extendedProperties.private.booking_id` so the 6 §14.2 listener cases can be exercised E2E. Next build wave. |
