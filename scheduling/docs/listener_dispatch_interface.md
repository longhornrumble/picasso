...
- **`workingLocation` is currently treated as OOO** (`eventType === 'workingLocation'` || `'outOfOffice'`). This matches §14.1's "OOO/working-location" phrasing, but **working-location (WFH/office/travel) is not absence** — it will fire false-positive `ooo_overlap_detected` against bookings. **Flagged for canonical §14.2 review:** confirm whether `workingLocation` should trigger overlap detection or be excluded. Until resolved, it is included + unit-tested. (audit row 12 — convergent)
