...
> **⚠ Platform Currency Reconciliation applied 2026-05-18 (+ multi-agent review, ratified).** This canonical was written ~2026-04-27 (committed PR #55). Inline `[Currency 2026-05-18: …]` markers show every statement reconciled against current platform state: provably-stale facts corrected in place; premise-dependent decisions initially flagged, then **reviewed by 4 independent agents and ratified** — see `[RESOLVED 2026-05-18 → Rn]` markers and **§ Platform Currency Reconciliation (2026-05-18) → Resolved decisions** at the end. Launch-gating items have a design-only plan: `launch_blocker_remediation_2026-05-18.md`. Reviewers/implementers: read the reconciliation section first.
...
2. **DB uniqueness scoped by `AppointmentType.format`.** Group format (when v2 lands) shares resource/start/end across N volunteer registrants; format-scoped uniqueness lets Group ship without schema migration. **Must be in v1 IaC and deployment checklist — not punted.** **[Currency 2026-05-18 — premise superseded; decision RESOLVED below → R1]** When written, no DynamoDB IaC pattern existed in the repo. As of **P0 Environment Foundation (complete 2026-05-18)** a Terraform-managed DDB module pattern now exists at `infra/modules/ddb-*` (~12–14 tables across ~12 modules, GSIs / TTL / PITR, OIDC CI auto-apply on push to the `staging` branch; **staging + dev accounts only — prod Terraform deferred to P0 Phase 2**). The existing `ddb-token-blacklist` module is the *session* blacklist (single `token_hash` key) — a **template/precedent**, not coverage of scheduling's distinct `picasso-token-jti-blacklist` (composite `tenantId`+`jti`), which shipped by runbook (PR #52) and is **not** in tfstate. "v1 IaC" therefore no longer means building a pattern from scratch (the ~1–2-day from-scratch cost is **void**); it means adding scheduling-table modules that follow the established `infra/modules/ddb-*` convention. Consequence: the implementation plan's task **A6 chose "runbook over IaC for v1" *because* this premise held**. **[RESOLVED 2026-05-18 → R1]** adopt the `infra/modules/ddb-*` Terraform pattern for remaining scheduling tables; `terraform import` the runbook-shipped jti table. See § Platform Currency Reconciliation (2026-05-18) → Resolved decisions → R1. Applies equally to all new scheduling tables (`Booking`, `AppointmentType`, `RoutingPolicy`, `ConversationSchedulingSession`, `picasso-calendar-watch-channels-{env}`, `picasso-token-jti-blacklist-{env}`).
...
Reuses existing platform infrastructure: SES for email. **[Currency 2026-05-18]** SMS has migrated off SNS to **Telnyx** (`SMS_Sender` / `SMS_Webhook_Handler` Lambdas, `picasso/telnyx` Secrets Manager credential, `picasso-sms-consent` TCPA table); SNS is now retained only for ops-alarm fan-out, not SMS delivery. The transactional-SMS foundation (`docs/roadmap/SMS_TRANSACTIONAL_BUILD_PLAN.md`, status *complete in staging*) already ships consent / STOP-HELP handling and 10DLC throttling (note: *staff*-notification quiet-hours only — **contact-facing reminder quiet-hours are NOT shipped and remain a sub-phase E build**, not inherited), **plus a scheduled-message primitive (`Scheduled_Message_Sender` Lambda driven by EventBridge Scheduler, whose header comment explicitly anticipates a calendar-integration caller)** — directly relevant to §11/§12 reminders and materially reducing sub-phase E backend scope. Existing conventions apply:
...
- (b) ~~Add cases for `start_scheduling` and `resume_scheduling` to the `intentLabel` switch in `prompt_v4.js`.~~ **[Currency 2026-05-18 — DONE]** Both cases now exist (`prompt_v4.js:898–899`; `intentLabel` switch at `:893`), returning `'SCHEDULE'`. The dependency audit's launch-blocker Risk #3 is **closed**.
- (c) ~~Add `start_scheduling` and `resume_scheduling` branches to the action-dispatch chain in `MessageBubble.jsx`.~~ **[Currency 2026-05-18 — DONE]** Placeholder dispatch branches landed at `MessageBubble.jsx:732–743` (sub-phase A) with test coverage at `MessageBubble.test.jsx:250`; they log + short-circuit so V4 Action Selector emissions don't silently fall through. Functional wiring remains intentionally deferred to sub-phases C/D per the branches' own comments. The dependency audit's launch-blocker Risk #4 is **closed**.
...
- **Failure alerting (resolved 2026-04-28, grounded in deployed state).** Scheduling routes alarms to the **already-deployed** SNS topic `picasso-ops-alerts` (`arn:aws:sns:us-east-1:614056832592:picasso-ops-alerts`), email-subscribed to `chris@myrecruiter.ai`. Verified via `aws sns list-topics` + `list-subscriptions-by-topic` on 2026-04-28. **[Currency 2026-05-18 — RESOLVED below → R5]** `614056832592` is the **prod** account. Post-P0 the platform is 3-account with a staging-first SOP, so staging scheduling alarms must target a *staging-account* SNS topic, not this prod-account ARN. MFS Phase 5.5 (2026-05-13) established staging alarm delivery (`mfs-phase5-alarms` SNS → AWS Chatbot → `#aws-alarms`). **[RESOLVED 2026-05-18 → R5; AMENDED 2026-05-25]** staging scheduling alarms target `arn:aws:sns:us-east-1:525409062831:picasso-ops-alerts-staging`; the prod ARN above is a Phase-2 prod-promotion step only. (The 2026-05-18 resolution originally named `mfs-phase5-alarms`, but `aws sns list-topics` against staging-525 on 2026-05-25 shows `picasso-ops-alerts-staging` is the only topic present.) See Reconciliation → Resolved decisions → R5. (Note: [`docs/roadmap/IT_OPS_ALERTING_STRATEGY.md`](../../docs/roadmap/IT_OPS_ALERTING_STRATEGY.md) proposes consolidating onto a renamed `myrecruiter-alerts` topic; that's roadmap, not implemented. If/when that consolidation lands, scheduling's alarm `--alarm-actions` references migrate as part of that effort.) **Scheduling will be among the first CloudWatch alarms wired to `picasso-ops-alerts`** — currently zero alarms route to it, so an early-life noise budget is not a concern. Three alarms required:
...
**What this is.** This canonical was written ~2026-04-27 (committed PR #55). The platform has moved since. This section is the audit trail for a **currency pass** (scope as agreed: refresh factual claims, file:line refs, dependency status, and effort sizing against what is now built; correct provably-stale facts in place; **flag** design decisions resting on now-false premises rather than re-deciding them). Ground-truthed 2026-05-18 against the live repo + `infra/` + roadmap docs by two independent read-only research passes.

**Verdict.** Scheduling remains a sound, well-specified v1 design. No design decision was invalidated outright. Two corrected facts *improve* the position (DDB IaC pattern now exists; transactional-SMS + scheduled-message foundation now shipped). Four items are referred to the reviewers (R1, R4, R5, plus naming R6). Two prior launch-blockers are **closed**.

### Corrected in place (provably-stale facts)

| # | Location | Was | Now (2026-05-18) |
|---|---|---|---|
| C1 | §5.2 item 2 | "no DynamoDB IaC pattern exists in the repo today… establishing the DDB IaC pattern from scratch (+1–2 days)" | **Superseded.** P0 (complete 2026-05-18) established `infra/modules/ddb-*` — ~12–14 Terraform-managed DDB tables w/ GSIs/TTL/PITR + OIDC CI auto-apply, staging+dev (prod deferred to P0 Phase 2). From-scratch cost is void; scheduling tables now follow an existing convention. *Reviewer-corrected:* existing `ddb-token-blacklist` is the **session** blacklist (precedent), **not** coverage of scheduling's `picasso-token-jti-blacklist` (shipped by runbook PR #52, not in tfstate — R1 must budget a one-time `terraform import`). |
| C2 | §6.3 Notifications | "SES for email, SNS for SMS" | **Email still SES (unchanged). SMS migrated to Telnyx** (`SMS_Sender`/`SMS_Webhook_Handler`, `picasso/telnyx` secret, `picasso-sms-consent` TCPA table). SNS retained only for ops-alarm fan-out. Transactional-SMS build *complete in staging*, incl. consent/STOP-HELP/10DLC + a `Scheduled_Message_Sender` (EventBridge Scheduler) primitive that explicitly anticipates a calendar-integration caller. *Reviewer-corrected:* contact-facing reminder **quiet-hours are NOT shipped** (only staff-notification quiet-hours exist) — they remain a sub-phase E build. |
| C3 | §6.2/§7 ride-alongs (b) | "Add `start_scheduling`/`resume_scheduling` cases to `prompt_v4.js:885-893` — not done" | **DONE.** Cases exist at `prompt_v4.js:898–899` (switch at `:893`). Dependency-audit Risk #3 **closed**. |
| C4 | §6.2/§7 ride-alongs (c) | "Add dispatch branches to `MessageBubble.jsx` ~702 — not done" | **DONE.** Placeholder branches at `MessageBubble.jsx:732–743` + test `MessageBubble.test.jsx:250` (sub-phase A landed them; functional wiring still deferred to C/D by design). Dependency-audit Risk #4 **closed**. |

### Re-verified — accurate, no change

| # | Location | Finding |
|---|---|---|
| V1 | §6.3 — `analyticsApi.ts:91-112` | Re-checked: file exists at `picasso-analytics-dashboard/src/services/analyticsApi.ts`; `buildHeaders()` @81, `X-Tenant-Override` @92, `buildAdminHeaders()` @102. Same minor line-drift the 2026-04-28 audit noted; pattern still present in the cited range. **Recorded so reviewers know it was checked, not missed.** |
| V2 | §14.1 — "no Google Calendar push-notification listener Lambda exists" | Re-verified 2026-05-18: still **accurate** — no calendar-watch/listener Lambda in `Lambdas/lambda/`. (Note: `Scheduled_Message_Sender` exists but is a reminder-dispatch primitive, not a calendar listener.) |
| V3 | §17 — "V5.0 is a naming convention, no V5 code" | Re-verified: still **accurate**. No V5/agentic/tool-use scaffolding exists. Phase 0 still *Initiation 2026-04-27, owner TBD, not started*. |

### Resolved decisions (ratified 2026-05-18 from multi-agent review)

Four independent reviewers (tech-lead, architect, security, PM) reached consensus on each item below; operator ratified 2026-05-18. These are now decisions, not flags.

- **R1 — RESOLVED: adopt Terraform.** Provision all remaining scheduling tables (`Booking` w/ 2 GSIs, `AppointmentType`, `RoutingPolicy`, `ConversationSchedulingSession`, `picasso-calendar-watch-channels`) via the `infra/modules/ddb-*` Terraform pattern. The already-shipped `picasso-token-jti-blacklist-staging` (runbook, PR #52, not in tfstate) is brought under management via a one-time `terraform import` (~30 min) before the first new scheduling module lands; `docs/runbooks/SCHEDULING_DYNAMODB_TABLES.md` becomes emergency-recovery reference only. **Precondition: R6 locked first.** Rationale: the from-scratch cost is void; the `Booking` two-GSI provisioning (un-addable retroactively) belongs under PR review, not a CLI runbook. Tech-lead + architect concur.
- **R2 — RESOLVED: sub-phase E working size 14–20 days** (was 20–28). The shipped Telnyx/`Scheduled_Message_Sender`/consent foundation removes ~3.5–5 backend days (E8 consent table, E3 dispatch Lambda, E2 scheduler mechanism). The UI surfaces (esp. E11 Calendar-Connection OAuth) + CI-6 synthetic monitor are the non-reducible bulk and must **not** be compressed. Mandatory task-level re-size at sub-phase E *entry*; 14–20 is the pre-entry planning number.
- **R4 — RESOLVED: V4→V5.0 (sub-phase G) working size 7–11 days.** §17's 5–7 is conditional on Phase 0 deliverable 2 (eval framework) existing; Phase 0 is unowned/not-started, so 7–11 is the honest number. G stays post-v1 (Austin pilots on V4). **G is not scheduled until Phase 0 deliverable 2 has a committed owner + start date** — the behavioral-parity harness is the only thing that makes a live-tenant migration verifiable.
- **R5 — RESOLVED 2026-05-18; AMENDED 2026-05-25: staging scheduling alarms target `arn:aws:sns:us-east-1:525409062831:picasso-ops-alerts-staging`** (staging account). The prod-account `…:614056832592:picasso-ops-alerts` ARN is a Phase-2 prod-promotion step only. This removes the cross-account isolation violation (CLAUDE.md hard rule). Applies to canonical §14.1 alarms 1–3 and impl-plan B7. (The 2026-05-18 resolution originally named `mfs-phase5-alarms`; `aws sns list-topics` on 2026-05-25 confirmed `picasso-ops-alerts-staging` is the only topic present in staging-525.)
- **R6 — RESOLVED: scheduling Terraform modules name tables `picasso-{name}-${var.env}`** (parameterized — resolves to `-staging` today, dev-ready, and participates in the Q3 rename sweep uniformly when it lands ~2026-07). No bare names; no hardcoded `-staging`. Lock before A8c (first scheduling table module). The already-shipped `picasso-token-jti-blacklist-staging` is consistent with this.

### Dependency status — unchanged since 2026-04-28 audit (re-confirmed 2026-05-18)

Consumer PII Remediation: was *Initiation 2026-04-27, owner TBD, not started* at the currency-pass snapshot — **subsequently RESOLVED 2026-05-18: owner = Chris; F0 scope = Path B for Austin v1; the full pipeline (Path A) is a separate parallel platform project gating tenant #2** (see launch-blocker cross-link below). High priority (live FTC §5 deceptive-claim exposure); scheduling is a **compounding, not originating** PII surface; Option C (ship Austin pilot defensively + run delete pipeline in parallel + hard-gate tenant #2) is the ratified posture. Agentic Phase 0: *Initiation 2026-04-27, owner TBD, not started*; not a blocker for the pipeline-driven Austin pilot; prerequisite for the agent layer (Phase 1+). The canonical's treatment of both remains valid.

### Reviewer corrections (2026-05-18, post-multi-agent review)

This reconciliation was itself reviewed by four independent agents (tech-lead, architect, security, PM). Corrections applied above: (1) DDB table count "15+" → "~12–14"; (2) C2 quiet-hours overstated — *contact-facing* reminder quiet-hours are NOT shipped (staff-only), they remain a sub-phase E build; (3) C1/R1 clarified — the existing `ddb-token-blacklist` Terraform module is the session blacklist (precedent), not coverage of scheduling's runbook-shipped `picasso-token-jti-blacklist`, so R1 must budget a `terraform import`. **Pre-existing doc bug surfaced (not currency drift, fix during implementation):** canonical §13.7/§16 informally describe the jti table as "keyed by `jti`" (single key) while the shipped runbook (`docs/runbooks/SCHEDULING_DYNAMODB_TABLES.md`) and §10 define it composite (`tenantId` PK + `jti` SK); align §13.7/§16 to the composite key. Full reviewer findings and the R1/R2/R4/R5/R6 recommendations are tracked outside this doc (consolidated gap matrix, 2026-05-18 review session). Launch-gating items have a design-only remediation plan: [`launch_blocker_remediation_2026-05-18.md`](launch_blocker_remediation_2026-05-18.md) — N1 (SQS dispatch queue / task B0a), N2 (PII governance / F0 gate — **RESOLVED 2026-05-18: F0 = Path B; owner Chris; Path A a separate parallel platform project gating tenant #2**), N3+N4 (Telnyx webhook replay-window + Secrets-Manager hardening).
