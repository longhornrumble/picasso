...
- **R5 — RESOLVED: staging scheduling alarms target `arn:aws:sns:us-east-1:525409062831:mfs-phase5-alarms`** (staging account, MFS Phase 5.5 pattern → AWS Chatbot → `#aws-alarms`). The prod-account `…:614056832592:picasso-ops-alerts` ARN is a Phase-2 prod-promotion step only. This removes the cross-account isolation violation (CLAUDE.md hard rule). Applies to canonical §14.1 alarms 1–3 and impl-plan B7.
