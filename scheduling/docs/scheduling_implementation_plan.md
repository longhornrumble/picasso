...
| B1 ✅ 2026-05-25 | Provision `picasso-calendar-watch-channels-{env}` DDB table via the **runbook pattern from A6** (corrected from earlier "terraform apply" wording — A6 chose runbook over IaC for v1), schema per canonical §14.1. **DONE staging-525:** table ACTIVE; PK `channel_id`; GSI `tenant-status-index(tenant_id, status)` ACTIVE; PAY_PER_REQUEST; SSE-DDB. Runbook: [`subphase_b1_calendar_watch_channels_runbook.md`](subphase_b1_calendar_watch_channels_runbook.md). PII inventory updated. Dev/prod deferred per cross-env note in runbook. | DevOps + lambda-orchestrator | ✅ Runbook commands succeeded; sample row write + GetItem + GSI query + cleanup all PASS |
