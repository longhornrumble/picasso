...
**Status column convention (added 2026-05-25 per audit row 19):** keep the # / Task / Primary agent / Verify check columns clean. Use the Status column for short milestones (`✅ YYYY-MM-DD` for done; `🟡` for partial; `—` for pending). Full completion details (table ARNs, GSI names, deploy SHAs, etc.) belong in the "Sub-phase B closure log" subsection below the table, NOT in the task cell.
...
#### B1 — `picasso-calendar-watch-channels-{env}` table (✅ 2026-05-25)
...
**Same-day audit-closure amendments (2026-05-25):** 2nd GSI `tenant-expiration-index(tenant_id, expiration)` added for B3 Renewer query path; PITR ENABLED (per pii-inventory invariant); tags re-aligned to platform `default_tags` convention (`Environment`/`Project=myrecruiter`/`ManagedBy`); dev-372 table provisioned with same schema so B2 unit tests have a target. See [`project_scheduling_subphase_b_opening_phase_completion_audit_2026-05-25.md`](../../../.claude/projects/-Users-chrismiller-Desktop-Working-Folder/memory/project_scheduling_subphase_b_opening_phase_completion_audit_2026-05-25.md) for full gap matrix + reviewer findings.

**B2 implementation gates (carried forward from B1 audit):** `channel_token` encryption decision (CMK item-level OR Secrets Manager + hash); IAM tenant-scoping (`picasso/scheduling/oauth/${tenantId}/*` parameterization before tenant #2). B2's Security-Reviewer pass cannot close without both.
