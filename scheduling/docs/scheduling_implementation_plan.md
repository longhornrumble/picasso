...
| D6 | Reschedule flow per §9.4: token redemption → land in chat with `rescheduling_intent` session-context binding (30-min TTL) → `proposing → confirming` → cancel old calendar event + create new (with Zoom `PATCH` to preserve join URL). **"Atomic" is aspirational — Google Calendar has no atomic-move API (per backend review 2026-05-02).** Concrete state machine handles the four real outcomes: (i) `events.insert` (new) succeeds + `events.delete` (old) succeeds → success. (ii) `events.insert` succeeds + `events.delete` fails → set `Booking.pending_calendar_sync = true` + store `rescheduled_old_event_id`; nightly reconciliation retries `events.delete` until success. Volunteer sees only the new invite (old one is silently lingering until reconciler kills it). (iii) ~~`events.delete` succeeds + `events.insert` fails → cancel~~ **CORRECTED 2026-06-02 (B-1, lambda#204): under insert-first + the delete-guard, this cell is UNREACHABLE** — if the insert fails, the delete is NOT attempted (a transient insert hiccup must never strand the volunteer with zero invites), so it folds into (iv). (iv) `events.insert` fails (delete skipped) → no state change; **retryable**; chat shows error, prompts retry. **Ordering decision: insert-first, delete-second + delete GUARDED on insert success.** Better two invites (recoverable via reconciler) — or zero CHANGE — than a destructive cancel on an infra blip. | Backend-Engineer + Frontend-Engineer | Unit tests for the reachable outcomes (i)/(ii)/(iv); state-flag transitions verified; reconciler-retry test for outcome (ii); **B-1 guard test: insert✗ → (iv) failed + `events.delete` NEVER called**; E2E happy-path: reschedule from email link → new time confirmed → old event canceled → join URL stable |
...
| E16 | **DESCOPED from v1 (operator decision 2026-06-12).** ~~Surface 9 — Calendar embed (Customer Portal): Google Calendar iframe embed on a "Calendar" tab.~~ Rationale: Google Calendar is the system of record for time (notifications, editing, mobile) — the dashboard's unique value is the booking *lifecycle* (E12), not time visualization; an embed rendering the coordinator's full calendar would pull non-Picasso event PII (titles/attendees) through our stack and contradict the Calendar tab's "Picasso never reads … other calendar data" trust copy. **Replacement:** each booking row deep-links "Open in Google Calendar" via `Booking.html_link` — read chain shipped with E12 (ADA projection + BookingCard render); the missing writer landed in lambda#291 (BCH persists `events.insert`'s `htmlLink`). Caveat accepted: the link opens in whichever Google account the browser is signed into. If pilots ask "why am I not bookable at X?", the v1.1 answer is a slots-debug preview driven by the real slot logic, not a calendar view. | — (descoped) | Booking rows render "Open in Google Calendar" when `html_link` present (BookingCard test); old rows without it render no link (ADA old-shape contract test) |
...
### Exit criteria
- All 17 tasks complete; verify checks pass (E1a, E1b, E2–E10, E11–E15 Customer Portal UI surfaces — E16 DESCOPED v1 per its task row (deep-link replacement, lambda#291) — CI-6 synthetic monitor).
