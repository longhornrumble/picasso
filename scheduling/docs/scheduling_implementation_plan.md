...
#### B5 — `Calendar_Watch_Onboarder` Lambda (🟡 stg, v1 pilot direct-invoke)
...
- **Infra PR (TBD)**: `lambda-calendar-watch-onboarder-staging/` Terraform module — Lambda function + dedicated IAM exec role + KMS-encrypted log group + 1 alarm (Errors). No Function URL (direct-invoke only); no SQS queues. Sources `listener_function_url` from the Listener module output so URL stays in lockstep.
...
**Operator smoke (deferred to post-merge):** `aws lambda invoke --function-name Calendar_Watch_Onboarder --payload '{"tenant_id":"MYR384719","coordinator_id":"test-coordinator"}' --profile myrecruiter-staging response.json` → returns `{channel_id, expiration, last_sync_token_seeded, secret_id}`; DDB row appears + Google `events.watch` returns active channel. Once smoke passes, B2 Phase 2b can begin with a real channel to validate against.
