...
**Phase 2b (pending, future session) — handler wire-up + typed-event derivation.** Turns `raw.calendar_push` envelopes into one of 7 typed events per [`listener_dispatch_interface.md`](listener_dispatch_interface.md). **New design dependency surfaced 2026-05-26:** Google Calendar push notifications identify a calendar, not a specific event — `events.list(syncToken)` is the standard delta-discovery primitive that must run between the push and per-event `events.get`. [`listener_dispatch_interface.md` "Delta Discovery" section](listener_dispatch_interface.md) documents the pattern + schema additions (`last_sync_token` + `coordinator_id` on `picasso-calendar-watch-channels-{env}` row). Phase 2b implementation lands those schema additions in B1 runbook + canonical §14.1 at the same time. CI-3b contract test lands with Phase 2b.

**Prod-614:** deferred to Phase-2 cutover per SOP; explicit + gated + rare.

#### B5 — `Calendar_Watch_Onboarder` Lambda (🟢 stg, v1 pilot smoke-verified 2026-05-27)
...
**Channel-token encryption gate (B1 audit row 12) CLOSED:** Onboarder is the first writer; implements Option 2 (Secrets Manager + SHA-256 hash). Matches Listener's compare-side that has been live since B2 Phase 1.

**IAM tenant-scoping gate (B1 audit row 13) carry-forward:** Onboarder exec role's `secretsmanager:GetSecretValue` is scoped to `picasso/scheduling/oauth/*` at v1 pilot scale; MUST be parameterized to `picasso/scheduling/oauth/${tenantId}/*` before tenant #2.

**Operator smoke — VERIFIED 2026-05-27.** Direct-invoke against `MYR384719/test-coordinator` registered channel `6f1fc1e2-264a-4cc3-b6cb-848de61694ce`: DDB row written with correct shape (tenant_id/coordinator_id/calendar_id=`primary`/status=`active`/`resource_id`=`7ssORZnKITkHFiSdykcqxAj5w_k`/`resource_uri`/`channel_token_sha256`), `expiration` = exactly created_at + 7 days (≈2026-06-03, matching Google's Calendar watch TTL ceiling). B5 done-bar all 3 conditions met: (1) DDB row present ✅; (2) Google watch registered (Google returned `resource_id` + `channels.stop` later returned HTTP 204 confirming the watch existed) ✅; (3) `expiration` within 7 days ✅.

**Phase-completion-audit 2026-05-28 + remediation.** 5 adversarial reviewers (code / tech-lead / test / security / lambda-orchestrator) → 26-row gap matrix. Owner disposition: fixed 2 blockers + cheap/high-value hardening; deferred the rest with rationale. Remediation: **lambda#171** (G1 expiration null-check; G3 `events.stop` compensation on post-watch failure; G4 regex allowlist on tenant_id/coordinator_id; G5 removed the OAuth client cache — root cause of the 2026-05-27 cold-start; **G6 removed the dead raw-token Secrets Manager store** → closed G10/G15/G25; G12 Jest coverageThreshold; G13 require CHANNELS_TABLE env; G16 WARN on null syncToken; G19 `singleEvents:false`; G21/G23/G24/G26; 74/74 tests, 100% coverage) + **picasso#271** (G2 **OAuth IAM scoped per-tenant via `var.scheduling_oauth_tenant_ids` — wildcard removed**; G6 dropped CreateSecret grant + channel-token namespace; G9 timeout 30→60s; G18 `reserved_concurrent_executions=2`; G8 placeholder-revert SHA-verify comment). **Deferred (audit memory `project_scheduling_b5_phase_completion_audit_2026-05-28`):** G7 tuple-idempotency → pre-B3; G8 structural placeholder fix → repo-wide; G11 push-delivery integration test → B2 Phase 2b; G22 X-Ray → observability; G14 ruled not-a-gap (table TTL is DISABLED, `expiration` is a plain data field). After both PRs apply, re-verify Lambda CodeSha256 ≠ placeholder + re-smoke.

**Test-channel teardown — 2026-05-28** (per [`subphase_b5_smoke_runbook.md`](subphase_b5_smoke_runbook.md) Step 3, Node-bridge pattern until B6 ships): `channels.stop` HTTP 204; DDB row deleted (`picasso-calendar-watch-channels-staging` now empty); channel-token secret scheduled for deletion (7-day recovery window, DeletionDate 2026-06-04). No residual active watch. B2 Phase 2b / B3 will mint a fresh channel when they need a live fixture.
