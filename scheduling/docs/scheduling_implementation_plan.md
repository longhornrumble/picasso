...
| B7 | CloudWatch alarms (3): Lambda errors on Renewer; custom `CalendarWatchRenewalFailed` metric; cron dead-man's-switch (>7h without successful run). All route to **`arn:aws:sns:us-east-1:525409062831:mfs-phase5-alarms`** (staging account) **[RESOLVED 2026-05-18 → R5]** — the prod-account `…:614056832592:picasso-ops-alerts` ARN was a cross-account isolation violation; corrected to the staging-account MFS Phase 5.5 topic. Prod ARN applies only at Phase-2 prod promotion. Reconciliation → Resolved decisions → R5 | DevOps | All three alarms in CloudWatch console; manual fire-test of each (send test SNS message; should land in chris@myrecruiter.ai) |
...
- **R5 — alarms → `arn:aws:sns:us-east-1:525409062831:mfs-phase5-alarms`** (staging acct). B7 updated; prod ARN is a Phase-2 step only.
