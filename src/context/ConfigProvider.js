import { createContext, useState, useEffect, useRef, useContext, createElement as h } from 'react';
...
const ConfigProvider = ({ children }) => {
  ...
  // Periodic config update checker with smart comparison
  const checkForConfigUpdates = async () => {
    ...
    // Skip update if chat has active messages
    if (window.picassoChatHasMessages) {
    ...
    try {
      ...
      // Only update if config actually changed (deep comparison)
      if (result.changed && result.config) {
        // Compare key fields to avoid unnecessary re-renders
        const currentVersion = config?.version || config?.generated_at;
        const newVersion = result.config?.version || result.config?.generated_at;
      ...
      // Don't log network errors as warnings - they're expected when offline
      if (!navigator.onLine) {
        console.log('📴 Offline - skipping config update');
      } else {
  ...
  useEffect(() => {
    const initializeConfig = async () => {
      ...
      // Start the config watcher AFTER initial load
      // This uses the _startConfigWatcher which respects active conversations
      _startConfigWatcher();
    };
  ...
  // Return using createElement instead of JSX
  return h(ConfigContext.Provider, { value: contextValue }, children);
};
