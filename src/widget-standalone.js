...
// Widget will access environment configuration from global window object
// This is set by the environment.js module when it loads
(function() {
  const PicassoWidget = {
    iframe: null,
    container: null,
    ...
    createIframe() {
      ...
      // Use build-time injected environment or fallback to runtime detection
      // Check for build-time constants first, then fallback to global window object
      const environment = typeof __ENVIRONMENT__ !== 'undefined' ? __ENVIRONMENT__ : 
                         (window.picassoConfig || window.PicassoEnvironmentConfig || {}).ENVIRONMENT || 'production';
      const isDevelopment = environment === 'development';
      const isStaging = environment === 'staging';
      const isProduction = environment === 'production';
      
      console.log(`🌐 Widget environment detected: ${environment.toUpperCase()}`);
      console.log(`📍 Using ${environment} configuration from environment.js`);
      
      // Allow explicit dev mode override regardless of environment
      const finalDevMode = explicitDevMode || (isDevelopment && isLocalhost);
      
      // Determine widget domain and iframe URL based on environment configuration
      let widgetDomain;
      let iframeUrl;
      
      if (finalDevMode || isDevelopment) {
        // Development: Use dev server (esbuild on 8000 or Vite on 5173)
        const protocol = 'http:';
        const hostname = window.location.hostname || 'localhost';
        // Check if we're using esbuild (script src contains 'dist' or port 8000/9000)
        const scriptSrc = document.currentScript?.src || '';
        const isEsbuild = scriptSrc.includes('/dist/') || window.ESBUILD_MODE === true;
        const port = isEsbuild ? '8000' : '5173';
        widgetDomain = `${protocol}//${hostname}:${port}`;
        iframeUrl = `${widgetDomain}/iframe.html?t=${this.tenantHash}`;
        console.log(`🛠️ Development mode: Using ${isEsbuild ? 'esbuild' : 'Vite'} dev server on port ${port}`);
      } else if (isStaging) {
        // Staging: Use current domain (localhost:3000 in testing, S3/CDN in deployment)
        widgetDomain = window.location.origin;
        iframeUrl = `${widgetDomain}/dist/staging/iframe.html?t=${this.tenantHash}`;
        console.log('🧪 Staging mode: Using current domain for widget frame');
      } else if (isProduction) {
        // Production: Use production CDN
        widgetDomain = 'https://chat.myrecruiter.ai';
        iframeUrl = `${widgetDomain}/iframe.html?t=${this.tenantHash}`;
        console.log('🚀 Production mode: Using production CDN');
      } else {
        // Fallback to production for unknown environments
        widgetDomain = 'https://chat.myrecruiter.ai';
        iframeUrl = `${widgetDomain}/iframe.html?t=${this.tenantHash}`;
        console.warn('⚠️ Unknown environment, defaulting to production');
      }
      
      // Store the expected origin for security validation
      this.widgetOrigin = widgetDomain;
      console.log(`🌐 Loading iframe from: ${iframeUrl} (${environment.toUpperCase()} mode)`);
    ...
    setupEventListeners() {
      window.addEventListener("message", (event) => {
        ...
        // Debug logging
        console.log("🔍 Debug - widgetOrigin:", this.widgetOrigin);
        console.log("🔍 Debug - event.origin:", event.origin);
        console.log("🔍 Debug - iframe exists:", !!this.iframe);
        console.log("🔍 Debug - iframe.contentWindow:", !!this.iframe?.contentWindow);
        console.log("🔍 Debug - event.source === iframe.contentWindow:", event.source === this.iframe?.contentWindow);
        ...
        // Temporarily disable strict source validation for debugging
        if (event.source !== this.iframe.contentWindow) {
          console.log("⚠️ Source mismatch, but allowing for debugging");
          console.log("🔍 Debug - event.source:", event.source);
          console.log("🔍 Debug - this.iframe.contentWindow:", this.iframe.contentWindow);
          // return; // Commented out to allow messages through
        }
