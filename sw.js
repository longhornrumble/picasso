...
self.addEventListener('fetch', (event) => {
  ...
  // Skip service worker for streaming endpoints to avoid buffering/caching
  // This ensures streaming responses bypass SW completely
  // CRITICAL: SW must not clone/buffer streaming responses or browser will batch
  if (url.href.includes('lambda-url.us-east') || 
      url.href.includes('streaming') ||
      url.pathname.includes('/stream') ||
      url.pathname.includes('/Master_Function') || // Lambda endpoint that may stream
      url.searchParams.get('action') === 'stream' ||
      url.searchParams.get('stream') === 'true' ||  // Check for stream=true param
      request.headers.get('Accept')?.includes('text/event-stream') ||
      request.headers.get('Accept')?.includes('application/x-ndjson')) {
    // Explicitly pass through to network without ANY SW processing
    // No caching, no cloning, no buffering, no interference
    // This is critical for real-time streaming to work properly
